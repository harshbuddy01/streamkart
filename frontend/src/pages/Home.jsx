import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import { ArrowRight, Headphones, BookOpen, Newspaper } from "lucide-react";
import { fetchProducts, fetchAuthors } from "../lib/api";
import { ProductCard } from "../components/ProductCard";
import { AuthorMarquee } from "../components/AuthorMarquee";

const HERO_IMG = "https://static.prod-images.emergentagent.com/jobs/26a13d95-9a81-4a7b-888d-0068c84116d6/images/2e5f7b9d4f5f17d4c5aa2355bd89702af151c78e3e5c9880385b45b3f3994958.png";
const NEWS_IMG = "https://images.unsplash.com/photo-1624930301503-8c4cacf3c8a6?crop=entropy&cs=srgb&fm=jpg&ixid=M3w4NTYxODF8MHwxfHNlYXJjaHwxfHxuZXdzcGFwZXIlMjB2aW50YWdlJTIwcmVhZGluZ3xlbnwwfHx8fDE3Nzc0NjExOTZ8MA&ixlib=rb-4.1.0&q=85";
const AUDIO_IMG = "https://images.pexels.com/photos/12955927/pexels-photo-12955927.jpeg";

export default function Home() {
  const [featured, setFeatured] = useState([]);
  const [bestsellers, setBestsellers] = useState([]);
  const [audiobooks, setAudiobooks] = useState([]);
  const [news, setNews] = useState([]);
  const [authors, setAuthors] = useState([]);

  useEffect(() => {
    Promise.all([
      fetchProducts({ featured: true, limit: 8 }),
      fetchProducts({ bestseller: true, limit: 6 }),
      fetchProducts({ category: "audiobook", limit: 4 }),
      fetchProducts({ category: "news", limit: 4 }),
      fetchAuthors(),
    ]).then(([f, b, a, n, au]) => {
      setFeatured(f);
      setBestsellers(b);
      setAudiobooks(a);
      setNews(n);
      setAuthors(au);
    });
  }, []);

  return (
    <div className="sk-paper" data-testid="home-page">
      {/* HERO — split editorial layout */}
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 pt-12 lg:pt-20 pb-20">
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-16 items-end">
          <div className="lg:col-span-7">
            <div className="text-xs tracking-[0.4em] uppercase text-[#722F37] mb-6">
              Volume I — Spring Collection 2026
            </div>
            <h1 className="font-serif-display text-5xl md:text-7xl lg:text-8xl leading-[0.95] text-[#1A1A1A]">
              The world's
              <br />
              <span className="italic text-[#722F37]">finest stories,</span>
              <br />
              quietly bound.
            </h1>
            <p className="mt-8 text-base md:text-lg text-[#4A4A4A] max-w-xl leading-relaxed">
              A curated bookstore for international readers. Discover books, audiobooks and news
              collections from Murakami to Márquez, from Atwood to Adichie — all in one
              well-kept library.
            </p>
            <div className="mt-10 flex flex-wrap gap-4">
              <Link
                to="/browse"
                className="inline-flex items-center gap-2 bg-[#722F37] text-white px-8 py-4 rounded-sm hover:bg-[#5a252b] transition-colors text-sm tracking-wider uppercase"
                data-testid="hero-cta-shop"
              >
                Browse Collection <ArrowRight size={16} />
              </Link>
              <Link
                to="/browse?category=audiobook"
                className="inline-flex items-center gap-2 border border-[#1A1A1A] text-[#1A1A1A] px-8 py-4 rounded-sm hover:bg-[#1A1A1A] hover:text-white transition-colors text-sm tracking-wider uppercase"
                data-testid="hero-cta-audio"
              >
                Listen to Audiobooks
              </Link>
            </div>
          </div>
          <div className="lg:col-span-5 relative">
            <div className="aspect-[4/5] overflow-hidden rounded-sm border border-[#D4AF37]/30">
              <img src={HERO_IMG} alt="StreamKart curated stack" className="w-full h-full object-cover" />
            </div>
            <div className="absolute -bottom-6 -left-6 bg-[#F9F6F0] border border-[#D4AF37]/40 px-6 py-4 rounded-sm hidden md:block">
              <div className="font-serif-display text-2xl text-[#722F37]">2,400+</div>
              <div className="text-[10px] tracking-[0.3em] uppercase text-[#1A1A1A]">Titles in stock</div>
            </div>
          </div>
        </div>
      </section>

      {/* AUTHOR MARQUEE */}
      <AuthorMarquee authors={authors} />

      {/* CATEGORIES — three pillars */}
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-20 grid md:grid-cols-3 gap-6" data-testid="categories-section">
        {[
          { slug: "book", title: "Books", desc: "Hardcover & paperback editions.", icon: <BookOpen size={20} /> },
          { slug: "audiobook", title: "Audiobooks", desc: "Narrated by world-class voices.", icon: <Headphones size={20} /> },
          { slug: "news", title: "News Collection", desc: "Curated journalism, monthly.", icon: <Newspaper size={20} /> },
        ].map((c) => (
          <Link
            key={c.slug}
            to={`/browse?category=${c.slug}`}
            className="group bg-white border border-[#EFE9DF] p-10 rounded-sm sk-card-hover"
            data-testid={`category-card-${c.slug}`}
          >
            <div className="text-[#722F37] mb-6">{c.icon}</div>
            <div className="font-serif-display text-3xl text-[#1A1A1A] mb-2">{c.title}</div>
            <p className="text-sm text-[#4A4A4A] mb-8">{c.desc}</p>
            <span className="inline-flex items-center gap-2 text-xs tracking-[0.3em] uppercase text-[#722F37] group-hover:gap-4 transition-all">
              Explore <ArrowRight size={14} />
            </span>
          </Link>
        ))}
      </section>

      {/* FEATURED — bento-ish */}
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-12">
        <div className="flex items-end justify-between mb-10">
          <div>
            <div className="text-xs tracking-[0.4em] uppercase text-[#722F37] mb-3">Editor's Selection</div>
            <h2 className="font-serif-display text-4xl md:text-5xl text-[#1A1A1A]">Featured this season</h2>
          </div>
          <Link to="/browse" className="hidden md:inline-flex items-center gap-2 text-sm tracking-wider uppercase text-[#1A1A1A] hover:text-[#722F37]">
            View all <ArrowRight size={14} />
          </Link>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6" data-testid="featured-grid">
          {featured.map((p) => <ProductCard key={p.id} product={p} />)}
        </div>
      </section>

      {/* AUDIOBOOK SHOWCASE — split */}
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-20">
        <div className="grid lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-5">
            <div className="aspect-[4/5] overflow-hidden rounded-sm">
              <img src={AUDIO_IMG} alt="Audiobook listener" className="w-full h-full object-cover" />
            </div>
          </div>
          <div className="lg:col-span-7">
            <div className="text-xs tracking-[0.4em] uppercase text-[#722F37] mb-4">Now playing</div>
            <h2 className="font-serif-display text-4xl md:text-6xl text-[#1A1A1A] leading-tight mb-6">
              Audiobooks, the way <em className="text-[#722F37]">authors intended.</em>
            </h2>
            <p className="text-[#4A4A4A] max-w-lg mb-8 leading-relaxed">
              Hear Murakami in Japanese cadence, Coelho in his own voice. Our audiobook catalog is
              chosen for performance as much as story.
            </p>
            <div className="grid grid-cols-2 gap-4">
              {audiobooks.map((p) => <ProductCard key={p.id} product={p} />)}
            </div>
          </div>
        </div>
      </section>

      {/* BESTSELLERS */}
      <section className="bg-[#EFE9DF] py-20">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          <div className="flex items-end justify-between mb-10">
            <div>
              <div className="text-xs tracking-[0.4em] uppercase text-[#722F37] mb-3">This week</div>
              <h2 className="font-serif-display text-4xl md:text-5xl text-[#1A1A1A]">Bestsellers</h2>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5" data-testid="bestsellers-grid">
            {bestsellers.map((p) => <ProductCard key={p.id} product={p} />)}
          </div>
        </div>
      </section>

      {/* NEWS COLLECTION */}
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-20">
        <div className="grid lg:grid-cols-12 gap-10">
          <div className="lg:col-span-4">
            <div className="aspect-[3/4] overflow-hidden rounded-sm">
              <img src={NEWS_IMG} alt="News collection" className="w-full h-full object-cover" />
            </div>
          </div>
          <div className="lg:col-span-8">
            <div className="text-xs tracking-[0.4em] uppercase text-[#722F37] mb-4">The Dispatch</div>
            <h2 className="font-serif-display text-4xl md:text-5xl text-[#1A1A1A] mb-4">
              International news, in print.
            </h2>
            <p className="text-[#4A4A4A] max-w-lg mb-10 leading-relaxed">
              From The Atlantic to Le Monde Diplomatique, our news collections bring the world's
              most thoughtful journalism to your reading chair.
            </p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-5" data-testid="news-grid">
              {news.map((p) => <ProductCard key={p.id} product={p} />)}
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
